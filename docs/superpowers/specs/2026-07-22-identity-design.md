...
Identity 定义单封验证码邮件发送端口。生产适配器使用环境变量配置的 SMTP 主机、端口、用户名、密码、发件地址和 TLS 模式；开发与自动化测试使用内存捕获适配器。
...
- SMTP 主机、端口、用户名、密码、发件地址、TLS 与超时；
- Web Cookie 名称、安全属性和允许的 Origin；
- 可信反向代理列表。

生产环境必需密钥缺失或安全属性不满足要求时启动失败。开发环境允许使用显式配置的内存邮件适配器，但不得静默退化或把验证码输出到普通日志。
