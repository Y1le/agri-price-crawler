...
func TestPostgresRepositoryLifecycle(t *testing.T) {
	...
	claimed, err := repository.Claim(ctx, "worker-1", runAt, 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := repository.Retry(ctx, id, retryAt, "temporary failure"); err != nil {
		t.Fatal(err)
	}
	if err := repository.Retry(ctx, id, retryAt, "temporary failure"); err == nil {
	...
	claimed, err = repository.Claim(ctx, "worker-2", retryAt, 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := repository.Complete(ctx, id); err != nil {
		t.Fatal(err)
	}
	if err := repository.Complete(ctx, id); err == nil {
	...
	claimed, err = repository.Claim(ctx, "worker-3", runAt, 1)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := repository.Dead(ctx, deadID, "permanent failure"); err != nil {
		t.Fatal(err)
	}
	if err := repository.Dead(ctx, deadID, "permanent failure"); err == nil {
...
func TestPostgresRepositoryValidatesInputs(t *testing.T) {
	...
	for _, limit := range []int{0, 101} {
		if _, err := repository.Claim(ctx, "worker", time.Now(), limit); err == nil {
